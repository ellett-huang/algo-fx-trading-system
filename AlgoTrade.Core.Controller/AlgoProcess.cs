    ...
    public class AlgoProcess : IAlgoProcess
    {
        ...
        public void Initialize(DataFeedInfo dataFeedInfo)
        {
            try
            {
                ...
                dataFeed.RetrieveHistoryData(TrackingSymbol, 1, 600).ForEach(x=>{
                    AddDataToBuffer(x);
                });
        ...
        private void AlgoAnalyizeClosePositions(float Similarity,float PorfitTaking)
        {
            ...
            (from C in positions
                         select C).ToList().ForEach(x =>
                    {
                        if (x.StopPrice!=x.CostBase && ((_currentPrice < x.StopPrice && x.PositionType == PositionType.Long)
                            ...
                            || (_currentPrice - x.StopPrice > PorfitTaking && x.PositionType == PositionType.Long))
                        {
                            ...
                            positions[positions.IndexOf(x)].StopPrice = _currentPrice;
                            LoggerManager.LogTrace("Met Porfit Taking condition , locked the profit and move the stop price to "+positions[positions.IndexOf(x)].StopPrice+"for " + TrackingSymbol);
             ...
             select t).ToList().ForEach(x =>
                     {
                         ...
                         {                             
                             ...
                             int count = (from C in positions
                                          where C.PositionType == x.PositionType && C.Symbol == x.Symbol
