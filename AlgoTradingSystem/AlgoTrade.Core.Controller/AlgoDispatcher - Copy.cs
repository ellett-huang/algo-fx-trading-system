...
namespace AlgoTrade.Core.Dispatcher
{
    public class AlgoDispatcher : IAlgoDispatcher, IDisposable
    {
        ...
        private bool tradeDecision(string PositionTypeName)
        {
            ...
            var Positions = from t in lastTransactions
                                where t.PositionTypeName == PositionTypeName
            ...
            {
                double PriceMargin2 = Positions.ToList()[2].Price == null ? 0 : (double)Positions.ToList()[2].Price;
        ...
        public List<TradingData> GetMarketHistoryData(string Symbol, int Interval, int Period)
        {
            List<TradingData> result = new List<TradingData>();
            try
            {
                DataFeed dataFeed = new DataFeed(dataFeedInfo);
                dataFeed.Connect(Symbol);
                result = dataFeed.RetrieveHistoryData(Symbol, Interval, Period);
                dataFeed.Disconnect();
                ...
                while (ordersQueue.Count > 0)
                {
                    ...
                    if (tradingOrder.ActionType == TradingAction.Buy || tradingOrder.ActionType == TradingAction.ShortSell)
                    {
                        ...
                        else
                        {
                            ...
                            if (tradeDecision(tradingOrder.PositionType.ToString()))
                            {
