    ...
    public class AlgoProcess : IAlgoProcess
    {
        ...
        public void OnTimer(object StateObj)
        {
            try
            {
                ...
                _tradingData = dataFeed.RetrieveData(TrackingSymbol, TimerInternal/60000);
        ...
        private void AlgoAnalyizeClosePositions(float Similarity,float PorfitTaking)
        {
            ...
            (from C in positions
                         select C).ToList().ForEach(x =>
                    {
                        ...
                        {
                            LoggerManager.LogTrace("Met Porfit Taking condition , closed positions for " + TrackingSymbol);
                            x.TradingDate = DateTime.Now;
                            if (x.PositionType == PositionType.Long)
                            {
                                x.ActionType = TradingAction.Sell;
        ...
        {
            if ((from p in positions
                 where DateTime.Now-p.TradingDate   < new TimeSpan(0, 10, 0)
                 select p
                ).Count() > 0)
                return true;
            else
        ...
        private void UpdateBuffer(TradingData tradingData)
        {
