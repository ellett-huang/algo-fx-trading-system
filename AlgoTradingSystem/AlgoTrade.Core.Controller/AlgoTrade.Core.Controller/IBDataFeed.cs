...
namespace AlgoTrade.Core.Dispatcher
{
    ...
    public class IBDataFeed : IDataFeed, EWrapper
    {
               ...
               // if (connection == null)
                {
                    ...
                    _connection.APIError += connection_APIError;
                    _connection.NewQuote += connection_NewQuote;
                    _connection.NewTimestamp += connection_NewTimestamp;
                    _connection.ConnectionStatus += connection_ConnectionStatus;
        ...
        public virtual void historicalData(int reqId, string date, double open, double high, double low, double close, int volume, int count, double WAP, bool hasGaps)
        {
                ...
                TradingData theTradingData = new TradingData()
                {
                    TradeDateTime = new DateTime(date),
                    Open = (float)open,
                    High = (float)high,
                    Low = (float)low,
                    Close = (float)close,
