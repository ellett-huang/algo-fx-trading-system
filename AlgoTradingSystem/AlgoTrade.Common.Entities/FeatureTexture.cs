...
namespace AlgoTrade.Common.Entities
{
    ...
    public class FeatureTexture
    {
        ...
        /// <returns></returns>
        public int Compare(FeatureTexture featureTexture, ref float result, bool LogDB = true)
        {
            return Compare(featureTexture, ref result, "",LogDB);
        }
        /// <summary>
        /// Return if all the features ALL above or ALL below the comparing one.
        /// </summary>
        ...
        /// <param name="LogDB"></param>
        /// <returns></returns>
        public int Compare(FeatureTexture featureTexture, ref float result, string Description,bool LogDB=true)
        {
            bool above = true;
            bool below = true;
            ...
            try
            {
                        ...
                        if (this.MACD[0] != null)
                        {
                            compareResult = GetBoolSimilarity<Feature>(this.MACD[0], featureTexture.MACD[0]);
                ...
                if(LogDB) 
                   LoggerManager.LogTrace(Description+" Algo Features Analysis, Value :" + resultMessage.ToString());
        ...
        private static float GetBoolSimilarity<T>(T First, T Second) where T : Feature
        {
            float result;
            if (First.Weight > 0)
            {
                ...
                if (result < 0)
                    return BaseConstants.MinSimilarity ;
                else 
                    return -BaseConstants.MinSimilarity ;
