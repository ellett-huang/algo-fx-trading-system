            ...
            try
            {
                if (Index > 0)
                {
                    if (Index > r * 2)
                    {
                        ...
                        float DX = 0f;
                        float ADX = 0f;
                        if (float.IsNaN(tradingDataSource[Index - 1].featureTexture.ADX.Value))
                        {
                            for (int j = r-1; j >= 0; j--)
                            {
                                for (int i = Index-j; i > Index -j- r; i--)
                                {
                        ...
                        else
                        {
                            ...
                            if (TR == 0)
                                return tradingDataSource[Index - 1].featureTexture.ADX.Value;
                            float PDI = DMP * 100 / TR;
                            float MDI = DMM * 100 / TR;
                            DX = Math.Abs(MDI - PDI) / (MDI + PDI) * 100;
